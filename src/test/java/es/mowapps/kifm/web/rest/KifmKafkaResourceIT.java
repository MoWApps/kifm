...
import es.mowapps.kifm.config.KafkaProperties;
...
class KifmKafkaResourceIT {
    ...
    void setup() {
        ...
        KifmKafkaResource kafkaResource = new KifmKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/kifm-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/kifm-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
